...
describe.dataset <- function(x){
  ...
  DataBibentry <- attr(myiris, "DataBibentry")
  ...
  #publisher <- 
  #if (publisher != "<not published>" ) {
  #  publication_line <- paste0("Published by ", attr(x, "Publisher"), " in ", 
  #                             attr(x, "PublicationYear"), " [", 
  #                             attr(x, "Identifier"), "]")
  #  cat(publication_line)
  #} else {
  #  cat("<not published>")
  #}
}
