---
title: "Making Datasets Truly Interoperable"
format: 
   html: default
   docx: default
   epub: default
...
nocite: |
  @dublin_core_dcmi_2020, @cyganiak_rdf_2014, @datacite_ev_datacite_2021, @beckett_rdf_2014, @datacite_metadata_scheme_4-4, @prov-o_2013, @DCAT_2, @biron_xml_2004, @N-Quads_2014, @N-Triples_2014
---
...
In a well-regulated working environment with large IT and documentation resources, our problems are well handled by extensive processes and procedures. GSIM is a statistical vocabulary that harmonises the uses of the SDMX statistical data standard [@sdmx_information_model_2021]
...
iris_bibentry <- bibentry( bibtype = "Misc", 
          title = "Iris Dataset", 
...
attr(myiris, "bibentry") <- iris_bibentry
attr(myiris, "row.names") <- NULL # for easier review of output
attributes(myiris)
...
```{r mybibentry}
mybibentry <- attr(myiris, "bibentry")
mybibentry$publisher
mybibentry$version
mybibentry$description
```
...
attr(myiris, "relatedItem") <- relatedItem
```
...
attr(myiris, "Date") <- myirisdata

```

```{r}
attr(myiris, "Date")$PublicationYear
attr(myiris, "Date")$Issued
```
...
#### The release General Method

The release general method would provide a similar support to the `devtools::release()` function: a step-by-step check of the data and metadata before making it public. In this case, of course, the release would be the dataset embedded in the data.frame-inherited R object.

As earlier stated, we differentiate between the concept of a dataset, which is similar to but not equal to the R object in the users runtime environment. While an abstract concept is R-language independent, an R object in memory is similar in the sense that it cannot be accessed by a third party.
...
If we serialise the dataset into an RDF resource, this information goes straight into the file; if we release a CSV file in concordance wih the W3C recommendations, we place this metadata into the accompanying JSON file. If we upload the CSV file or the SPSS file to Zenodo, we are expected to fill out this information on Zenodo by hand or the uploading utility we use for this purpose.
...
Again, it is unlikely that somebody wants to edit such statements in R, however, once the statements are clearly recorded, it may be a good idea to attach them back to the R object or its `.rds` file representation.
